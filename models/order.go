...
type Order struct {
	...
	ActualPrice    string `json:"actual_price" xorm:"not null default 0.00 comment('实际需要支付的金额') DECIMAL(10,2)"`
	...
	OrderPrice     string `json:"order_price" xorm:"not null default 0.00 comment('订单总价') DECIMAL(10,2)"`
	GoodsPrice     string `json:"goods_price" xorm:"not null default 0.00 comment('商品总价') DECIMAL(10,2)"`
	...
	FreightPrice   int    `json:"freight_price" xorm:"not null default 0 comment('配送费用') INT(10)"`
	CouponId       int    `json:"coupon_id" xorm:"not null default 0 comment('使用的优惠券id') MEDIUMINT(8)"`
	ParentId       int    `json:"parent_id" xorm:"not null default 0 MEDIUMINT(8)"`
	CouponPrice    string `json:"coupon_price" xorm:"not null default 0.00 DECIMAL(10,2)"`
	CallbackStatus string `json:"callback_status" xorm:"default 'true' ENUM('false','true')"`
}
